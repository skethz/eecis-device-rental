...
do $$ declare n int; begin
  select count(*) into n from devices; if n <> 49 then raise exception 'expected 49 devices, got %', n; end if;
  select count(*) into n from devices where labelled; if n <> 35 then raise exception 'expected 35 labelled devices, got %', n; end if;
  select count(*) into n from devices where not labelled; if n <> 14 then raise exception 'expected 14 unlabelled devices, got %', n; end if;
