...
1. **Request.** A borrower signs in, picks a device, and fills in the request form (dates, their name, and their group picked from the group registry). The form shows the chosen group's lab manager and professor (name and email) so the borrower can check them; these are copied into the rental when it is submitted, so later edits to the group do not change past rentals. A group that is not listed is added by the lab manager (see *Editing groups* below). The name field offers a type-or-click picker fed by the [IIS people page](https://iis.ee.ethz.ch/people.html) (static copy in `site/people.js`, regenerated by `scripts/fetch_people.py` and weekly by the `refresh-people` workflow); people not on the list can still be typed in. Submitting sends an email to the lab manager with an **Approve** and a **Deny** link. Clicking either link opens a confirmation page on the website ("you are about to approve/deny this") — nothing changes until you press the button on that page. Once decided, the borrower gets an email with the outcome.
...
- **Editing inventory.** Sign in as an admin and open the *Devices* page (`site/devices.html`); the **Edit devices (admin)** section at the bottom is visible only to admins. It lists every device — inactive ones included — with an editable row for name, maker, model, unit number, *labelled* and *active*, plus an "Add a device" form. Devices are never deleted: untick **Active** to retire one so the rentals referencing it keep working. You can still edit the `devices` table directly in the Supabase Table Editor, or update `EECIS_DEVICE_LIST.xlsx` and regenerate the seed SQL:
  ...
  supabase db push
  ```
- **Editing groups.** The **Groups (admin)** section on the *Devices* page lists every group — inactive ones included — with an editable row for the group name, its lab manager's and professor's name and email, and *active*, plus an "Add group" form. Groups are never deleted: untick **Active** to hide one from the request form. The data lives in the `groups` table (0010); insert/update is allowed only when `public.is_admin()` returns true.
...
supabase/migrations/     schema, RLS policies, RPCs, seed data, webhooks/cron (0001-0005),
                         ...
                         device proposals (0007), Taekwang inventory devices (0008-0009),
                         group registry (0010)
